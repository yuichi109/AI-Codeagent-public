...
- [ ] **Phase 2: ハイブリッドモード**（★★★）（#27）
  - Qwen2.5 7B をオーケストレーター（日本語会話・判断担当）として動作
  - `delegate_to_azure` ツールを追加 — コード・ツール作業と判断したらgpt-4.1に委譲
  - Qwen2.5を採用する理由: VRAM 8GB制約 + CJK特化で日本語が自然
  - 実装順: #25（ツールトグル）でQwen2.5の動作を検証後に着手
...
#### ローカルLLM関連
- [ ] **ローカルLLM ツール ON/OFFスイッチ**（#25）: 現在はローカルモデル全てでツール無効。Llama3.1/Mistral/Hermes等はツール対応済みなので、⚙️パネルにトグルを追加して個別に有効化できるようにする
