...
- **returncode が 0 以外、または stderr にエラーがある場合:**
  - **cp コマンドが "No such file or directory" で失敗した場合のみ** 以下を実行する:
    1. `ls ~/AI-Codeagent/workspace/_gp_tmp` を実行してクローン済みのディレクトリ一覧を取得
    2. **番号付きリスト**で表示し「どのディレクトリを取り出しますか？ 番号を入力してください:」と聞く
    3. ユーザーが番号を選んだら cp コマンドを正しいディレクトリ名で再実行する（**リトライは1回のみ**）
    4. 再実行も失敗したら即停止してエラーをそのまま報告する
  - **それ以外のエラー（clone失敗・rm失敗等）は即停止してエラー内容をそのまま報告。リトライ禁止**
- **4コマンドが全て成功したら、必ず次のステップ5に進む。完了報告を先に出してはいけない**

### ステップ5: git管理の有無を確認する（**必須・スキップ禁止**）

ステップ4完了後、**必ず**ユーザーに確認する：
