...
- **【出力先の厳守】作成・編集するファイルは、タスク指示の冒頭に列挙された正確なパスのみ**。
  この役割プロンプトの「## 出力先」に書かれた `code/` 等の一般的なパスは**今回は無効**で、タスクで指定されたパスが優先です。
  指定外の場所（`code/` 配下など）にファイルを作らないこと。指定パスにそのまま作れば検収を通ります。
