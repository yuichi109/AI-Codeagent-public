...
- **原因（手順の順番が逆）**: スキルが「手順4で `tar czf`（=上書き）→ 手順5で既存なら確認」という順序になっていた。`tar czf` は既存ファイルを無条件で黙って上書きするため、手順5の確認時には**すでに上書き済み**で手遅れだった。
- **修正**: バックアップ手順を「**圧縮の前に** `test -f ~/Backups/<file>.tar.gz` で存在確認 → 既存なら y(上書き)/t(時刻付き別名) をユーザーに確認してから tar」に並べ替え。`tar czf` は無条件上書きなので必ず圧縮前にチェックする旨を明記。スキルは再起動不要で即反映。

---

## 2026-06-22（セッション61）`/backup-subdir` の列挙対象を明示（v1.20.9）

- **症状**: gpt-5.4-mini で `/backup-subdir` を実行すると、workspace ルートではなく**スコープ中のサブディレクトリ（`workspace/AI/` 内の `__pycache__`・`templates`）を列挙**してしまう。GPT-4.1 は workspace ルート直下を正しく列挙。
- **原因**: スキルが `list_files()` の**対象パスを明示していなかった**ため、セッションのスコープ（`workspace/AI/`）の指示に弱いモデルが引っ張られ、ルートではなくスコープ内を列挙した。`list_files(".")` は `_resolve_safe_path` により**常に workspace ルート**を指す（スコープ非依存）。
- **修正**: `skills/backup-subdir/SKILL.md` の手順1を **`list_files(".")` を名指し**＋「スコープが AI 等でも必ず workspace ルート直下を対象にする」明記に変更。A（名指し発火）と同じ「弱いモデルに探させ／解釈させない」方針。スキルは再起動不要で即反映。

---

## 2026-06-22（セッション61）スキル導線の強化：名指し発火＋補完キャッシュ解消（v1.20.8）

- **背景**: `/backup-subdir` を既定の gpt-5.4-mini で呼ぶと「用意されていません」と誤答（上位モデルでは成功）。スキルはシステムプロンプトに正しく入っている（`/skills`・`_load_skills` で確認済み）が、**長いスキル一覧から非力なモデルが新規スキルを拾い損ねる**のが原因と切り分け。
- **A: `/スキル名` の名指し発火**（`prompts.py` + `server.py`）: 新ヘルパー `load_skill_by_trigger(text)` を追加し、ユーザー入力の先頭トークンが `/トリガー` に一致したら、`agent_stream` でそのスキル本文をシステムプロンプト末尾に「🎯 ユーザーが呼び出したスキル」として**名指しで前面注入**。一覧から探させないので mini でも確実に発火する。frontmatter の `trigger:` で照合・`{APP_PORT}` 置換も適用。
- **B: `/` 補完キャッシュ解消**（`index.html`）: 補完ポップアップを開く瞬間に `_skills` キャッシュを破棄して `/skills` を取り直す。新規作成したスキルがページ再読込なしで補完候補に出るようになった（従来は初回取得をセッション中ずっとキャッシュしていた）。
- テスト: `load_skill_by_trigger` が `/backup-subdir 1,3`・`/commit` で本文を返し、通常文・未知トリガーは None を返すことを確認。py_compile OK・サービス再起動済み。

---

## 2026-06-22（セッション61）`/backup-subdir` スキル追加（v1.20.7）

- **追加**: `skills/backup-subdir/SKILL.md` — workspace 直下のサブディレクトリを個別の tar.gz でバックアップ／リストアする対話式スキル（番号選択・上書き/別名確認・退避付きリストア）。
- **shell=False 対策を明記**: `run_command` は `shell=False` のため、①シェル変数展開（`$(date)`/`$HOME`）②**グロブ（`*`）** がいずれも効かない。日付は `date +%Y%m%d` で取得して埋め込み、一覧は `ls -lh ~/Backups`（または `find ~/Backups -name '*.tar.gz'`）を使う旨をスキル内に記載。`*` を直接 `ls` に渡すと「No such file」で失敗する点を検証済み。
- tar は時間がかかるため `run_command` に `timeout_minutes`（例10）を指定して既定30秒で打ち切られないようにする旨も明記。
- 保存先は `~/Backups/`（workspace 外だが run_command の出力先は scope チェック対象外＝rm系と work_dir のみ制限のため書ける）。`~` 展開・`&&` 自動分割が効くことも確認済み。

---

## 2026-06-22（セッション61）_stream_command のタイムアウト不発（スピナー残り）修正（v1.20.6）

- **症状**: ブラウザでエージェントにサーバー再起動等をやらせると、処理は完了しているのに**ツールのスピナー（🌀 Running: ...）が消えず回り続ける**（デッドロックではない＝ブラウザは生きているが SSE が閉じない）。
- **真因**: 実経路 `server.py` の `_stream_command`（run_command のストリーミング実装・同期版 run_command とは別物）が `async for line_bytes in proc.stdout` で**次の行（EOF）を待つ**構造で、**タイムアウト判定がループ内**にあった。そのため「出力を出さず終わらないコマンド」（再起動直後の疎通確認 `curl http://localhost:8080/` が閉じない等）を踏むと、ループ本体に一度も入れず**タイムアウトが発火しない** → SSE が永久に開く → フロントの `statusEl.remove()`（`finally`＝ストリーム終了時のみ実行）に到達せずスピナーが残る。`sleep 30`＋3秒タイムアウトで12秒超ハングを実証。
- **修正**: `_stream_command` を `proc.stdout.readline()` を `asyncio.wait_for(timeout=0.5)` で待つループに変更し、**出力ゼロでも deadline とプロセス終了を毎回チェック**。タイムアウト時は `start_new_session=True`＋`os.killpg`（新ヘルパー `_kill_proc_tree`）で**プロセスグループごと kill**（孫の curl 等も道連れ・独立セッション化で本体に波及せず）。`proc.wait()`/stderr 読みも `asyncio.wait_for` でガード。`stdin=DEVNULL` も付与。
- v1.20.5（同期版 run_command の EOF ハング修正）の**ストリーミング版への適用漏れ**を解消。これでブラウザ経由でも「終わらないコマンド」で必ずタイムアウトして SSE が閉じ、スピナーが消える。
- **テスト**: `sleep 30`＋3秒→3.0秒で戻る／正常系 `./restart.sh`→5.4秒 rc=0 完走。両方合格。実機サービス再起動・ユーザー確認済み（「ちゃんと行けた」）。両ブランチ push 済み。
...
- **原因（回帰）**: セッション58の `975d3e2` で `body` に `overflow-y: hidden` を**明示**したこと。それ以前は `overflow-x: clip` のみで `overflow-y` 未指定 → CSS仕様により一方が `clip` だと他方の `visible` は `auto` 扱いになり、**内容がはみ出すと自動で縦スクロールバーが出ていた**。`hidden` 明示でこの救済が消え、レイアウトがビューポート高を超える環境（Server2025のDPI/描画差）で到達不能になった。
- **修正**: `index.html` の `body` を `overflow-y: hidden` → `overflow-y: auto`（`overflow-x: clip` は維持しヘッダー横ガタつき対策は保持）。収まっている環境（Win11）では挙動不変・スクロールバーは出ない。
- 両ブランチ push 済み（main 41d2ae6 / for_windows 5a438dc）。
...
- **症状**: シェル等の右ペイン（および履歴の左ペイン）を開くと、ヘッダーのアイコン群に**上端が被る**。原因はアイコン化でヘッダー内ボタンが 24px→32px に高くなりヘッダー全体が約45px→約53pxに増えたのに、パネルの固定 `top:44px` が旧高さのままだったこと。さらに最上部の `#workspace-scope-bar`（約30px）も加味されていなかった。
- **修正**: 5パネル（gitlab/provider/todo/shell/sessions）の `top:44px`・`height:calc(100vh-44px)` を CSS変数 `--pane-top` 化。`syncPaneTop()` が起動時・`load`・`resize` で「scope-bar＋header の実測 offsetHeight」を測り `--pane-top` に設定。**ヘッダー高さが将来変わっても自動追従**するため固定値ズレが原理的に起きない。

### v1.15.0 ヘッダーをアイコン統一＋自前ツールチップ＋4テーマ切替

- **目的**: 上部ヘッダーが絵文字＋テキストのボタン11個並びで「下部バーと同じくダサい」というユーザー指摘。下部バー刷新（v1.14.2）と同じ流れで、まずモック提示→ユーザーが案A（アイコン統一・機能は全部残す）を選択→実装。
- **見た目**: 🖥/📝/🕐/⚙️/🦊/📋/🔊/✏️/🧹/⏰/履歴クリア を 18px ラインアイコン（32px正方ボタン `.cbtn` 再利用）に統一。旧テキストボタン時代に各ボタンID個別の `background:#45475a` 等が残っていて「枠が出るボタン/出ないボタン」が混在していたのを撤去し透明に統一（機能状態＝tts点灯・clear赤ホバーは残す）。
- **自前ツールチップ**: `title` 属性を全廃し `[data-tip]` 属性ベースの自前ツールチップに統一（ホバー即時表示・Catppuccin配色・ヘッダーは下向き）。下部バーの `.cbtn` 5個も `title→data-tip` に揃えた。
  - **はみ出しバグ対処**: 右端付近のボタン（定時/読み上げ/掃除/履歴クリア）はツールチップが画面右端を超え、横スクロールバー出現→レイアウトシフト→ホバー外れ→…のループでちらつく問題。①該当ボタンのツールチップを右寄せ（`right:0`）②`body { overflow-x: clip }` で横スクロールバー自体を根絶（縦のツールチップ・子のスクロールコンテナには無影響）の2段で根本対処。
- **アイコンテーマ選択**: ヘッダーアイコンの見た目を4スタイルから選択可。`#header[data-htheme]` 属性でCSS切替・`localStorage(headerTheme)` 保存・再起動不要・**既定は色分け**。
  - `mono`（フラットモノクロ＝従来）/ `color`（カテゴリ別アクセント色）/ `group`（関連ボタンを背景パネルでまとめる・`.hgrp` ラップ）/ `outline`（全ボタンに薄枠）。
  - group テーマのため、ボタンを役割で3グループ＋単独 clear に再構成（並び順が変わった）: 作成・編集=シェル/エディタ/Draw.io、履歴・設定=履歴/LLM/GitLab、実行・管理=タスク/定時/読み上げ/掃除、単独=履歴クリア。
  - 選択UIは ⚙️ LLM設定パネル下部に「ヘッダーの見た目」4ボタン（選択中はハイライト）。
- **アクセシビリティ**: `title→data-tip` 化で標準ツールチップが消えた分、起動時に `data-tip` を `aria-label` へ自動反映（支援技術がアイコンのみボタンの名前を読めるように）。将来追加の data-tip 要素も自動カバー。
- **GitLabアイコン**: 汎用 git-branch では「GitLab」と分かりにくいため公式タヌキロゴ（fill currentColor）に差し替え。
- **実装**: `index.html` のみ（CSS テーマ群＋ツールチップ、`applyHeaderTheme`、起動時IIFE 2本）。サーバー変更なし。バッジ（タスク/定時）はアイコン右上の絶対配置に調整。
- **既知の設計上の注意**: ネイティブ `<select appearance:none>` の黒塗り不具合（[[project-status]] v1.14.2）を踏まえ、テーマ選択UIは自前ボタン方式。上部モデル選択はネイティブ select のままで正常。

---

## 2026-06-11（セッション45）別モデルで再実行バー（プロバイダー横断・v1.14.0）＋ BG/定時タスクの時刻表示修正（v1.13.13）

### v1.14.2 チャット下部バーのデザイン刷新（案A）＋ ネイティブ select 黒塗り不具合の根治

- **目的**: 下部入力バーが絵文字バラバラ・ボタン形状不統一・統計テキスト折り返しで見栄えが悪かった。アイコン統一でスッキリさせる（案A：機能は全部残す方向）。
- **見た目**: 📎/`</>`/💾/🖼/🎤 を 18px ラインアイコン（32px正方ボタン）に統一。直接保存・生成元画像はアイコンのみ＋ツールチップ。区切り線追加。統計テキストは小さく折り返し無し。送信＝青塗り＋紙飛行機、BG＝雷アイコン付きチップ。
- **不具合根治（重要）**: モード切替・マルチAI品質の `<select>` が、この環境（Windows系ブラウザ）で **`appearance:none` を付けると初期状態で文字が描画されず黒塗り→クリック（フォーカス）で出る**という症状。他者には操作項目の存在が分からず質が悪い。`appearance:none` 除去・背景画像除去・`-webkit-text-fill-color` を順に試すも `display:none→表示` 切替の select 等で残存 → **ネイティブ select を全廃し、自前のボタン＋ポップアップメニュー方式に置換**して原理的に解消。
  - モードの真実値は元々 localStorage `agentMode`（select は表示のみ）だったため置換が容易。`updateModeSelect` は自前ボタンの更新に変更。
  - **おまけ修正**: マルチAI品質選択が保存キーを `agentMode`（モードと共用）にしていて常に「バランス」に戻る壊れ状態だった → 専用キー `maQuality` にして正しく保存・選択。
- **実装**: `index.html` のみ（CSS `.cbtn`/`.dd`/`.dd-menu` 等、`_initDropdown` 共通関数）。サーバー変更なし。

### v1.14.1 再実行を「一時的」に（上部プロバイダー選択を変えない）

- **症状/要望**: v1.14.0 では再実行時にサーバーのグローバル設定ごと切り替え、**画面上部のモデルプルダウンまで変わって**いた。ユーザー要望「再実行は一時的なので上部は変えたくない」。
- **修正**: 再実行は `スナップショット → モデル一時切替 → 実行 → 復元` 方式に変更。`updatePvCurrent` を呼ばず**上部UIは一切触らない**。
  - サーバー新設 `POST /providers/snapshot`（現設定を退避しトークン返却）/ `POST /providers/restore`（トークンで復元）。**api_key 含むフル設定を退避**するため、ローカル/カスタム接続起点でも確実に元へ戻せる（preset では戻せないケースを解消）。
  - 通常ターンは `sendMessage` をストリーム完了まで await してから復元。BG はジョブ作成時に設定が固定されるので作成後すぐ復元（`_rerunBgBusy` で多重防止）。
  - 回答ラベルは再実行先プロバイダー名＋モデルを表示（`_rerunProviderName`、横断時の名称ズレも解消）。
- **実機検証**: ローカルLLM（openai_compatible）選択中に Azure gpt-5.4-mini で再実行 → 正答、上部はローカルのまま、次の通常送信はローカルで実行。snapshot→switch→restore の round-trip も `RESTORED_OK` 確認。

### v1.14.0 各ターン・BGカードに「別モデルで再実行」バーを追加（プロバイダー横断）

- **目的**: 「nano で投げてダメ → 気づいて mini に切り替えて再実行」を毎回手動でやっていたのを 1クリック化。失敗時だけでなく「結果に不満」なときも別モデルで投げ直せる。
- **UI**: 各ターンの AI 回答の下（＋ ⚡ 自分で投げた BG 結果カードの下）に `⟳ 再実行` ＋ モデルプルダウン。**普段は透明、ホバーで点灯**（チャットを汚さない）。`⏰ 定時タスクのカードには出さない`（定時UIの実行ボタンを使う方針）。
- **横断対応**: プルダウンは**マルチAI設定の割当モデルを横断・重複排除**したリスト（サーバー新設 `GET /rerun-models`）。複数プロバイダーなら optgroup でグループ表示。`未設定なら現在プロバイダーのモデル一覧にフォールバック（B案）`。
- **再実行の流れ**: `preset切替（/providers/preset）→ モデル指定（/providers/deployment）→ 同じ依頼を再実行`。`/providers/deployment` は現プロバイダーのモデルしか受け付けないため**順序必須**。通常ターンは `sendMessage`、BG は `bgSubmitJob` で投げ直す。
- **選択の永続化と防御**: 選んだ `preset_id::model` を localStorage(`agentRerunModel`) に保存し次回もデフォルト表示。`毎描画で現リストに存在チェック→無ければ破棄`してフォールバックするので、セットアップでマルチAIのモデルを変えても壊れない。
- **フック**: `updatePvCurrent`（起動時＋全プロバイダー切替で通る）から `refreshRerunModels()` を呼び、リスト更新と既存バーの作り直しを一括化。
- **実装**: `server.py`（`/rerun-models`）, `index.html`（`_buildRerunBar`/`_applyRerunModel`/`_attachRerunBar`/`_attachBgRerunBar`/`refreshRerunModels`、CSS `.rerun-bar`）。
- **検証済み**: Azure gpt-5.4-mini → Gemini gemini-2.5-flash → Azure gpt-5.4 の横断再実行を実機確認。**ローカルLLM起点での「ローカル→クラウド」逃がしは未検証（次セッションで確認）**。

### v1.13.13 定時タスクのチャット時刻を実時刻に・BGペインに日時表示

- **症状**: 定時タスクのチャットカードの時刻が、別ブラウザや翌日に開くと「過去のジョブなのに今の時刻」になっていた。原因はポーリングで気づいた時刻（`new Date()`）をスタンプしていたため（`_schedNotified` がブラウザ毎の localStorage）。
- **修正**: 実行記録の `decided_at`/`scheduled_at`（サーバー値）を使うよう変更（[index.html] `schedPollResults`）。今後実行分は正しい時刻に（過去分は方針Bで放置）。
- **追加**: BGペインのジョブカードに日時を表示（投入🕒/完了✓、UTC→JST変換、`_bgFmtTime`/`_bgSetTime`）。見やすいティール色。
...
### v1.13.12 インストール後の余計な Enter を撤去（1窓で止まらず最後まで流れる）

- **症状**: 1窓化（v1.13.10）後も、ツール導入が終わった所で `install_prereqs.ps1` 末尾の `Read-Host`（Press Enter to close）が残っていて、**続行に Enter が必要**だった（別窓時代の名残）。
- **修正**: その `Read-Host` を削除（`Continuing setup...` 表示のみ）。インライン呼び出しなので止める必要がなく、venv→pip→起動まで**中断なしで自動継続**。停止は **start.bat 末尾の完了画面（URL表示＋pause）の1回だけ**に統一。
...
- **修正**:
  - **昇格が必要なときは `start.bat` 全体を1枚の管理者窓で開き直す**（`Start-Process -FilePath '%~f0' -Verb RunAs`）。インストール〜venv〜pip〜.env〜起動まで**全部その1窓に出力が流れ続ける**。元の窓は「新しい管理者窓で続行します。この窓は閉じます」と案内してから終了（＝真っ黒待ちが無くなる）。UACキャンセル時は手動案内＋pause。
  - 既に管理者なら開き直さずその場でインライン実行。
  - `pip install` の `--quiet` を撤去し**進捗を表示**（空白の待ち時間をなくす）。
  - **初回は最後に「Setup complete! / http://localhost:<port>」を表示して `pause`**（キー押下まで閉じない・URLは `.env` の APP_PORT から取得）。通常起動（2回目以降）は従来どおりトレイ起動して即閉じ。
- **トレードオフ**: 初回のみトレイ/サーバーが管理者権限で起動する（2回目以降は venv 既存のため一般ユーザーで起動）。1窓で迷わない体験を優先した。
- **検証**: 純ASCII維持・非管理者分岐とポート読み取り（`.env`→URL）をスタブ実機で確認。
...
- **症状1（文字化けでコマンド誤実行）**: 実機（Windows Server 2025）で `start.bat` 実行時、先頭付近で `'…本体）へは進まない' は…コマンドではありません` のように **`::` コメント行の一部がコマンドとして実行**されてしまう。
  - **原因**: `chcp 65001`（UTF-8）の既知 cmd バグ。UTF-8 マルチバイトのコメント/メッセージが累積すると cmd の読み取り位置がズレ、ある `::` 行の先頭が欠けて中身が実行される。Windows ビルド依存で出る/出ないが、Server 2025 では再現。
  - **修正**: `start.bat` を **完全 ASCII 化**（コメント・メッセージとも英語、`chcp` 撤去）。非ASCIIバイト 0・BOM 無しを検証。これで cmd の codepage 依存を完全排除し、どの Windows でも再発しない。アプリ本体UI（ブラウザ側）は日本語のまま。
- **症状2（winget でインストール不可）**: 昇格は成功するが winget が3本とも `0x8a15000f : ソースに必要なデータがありません`（`SOURCE_DATA_MISSING`）で失敗。
  - **原因**: Windows Server は winget のパッケージソース（インデックス）が未同期のまま。
  - **修正**: インストール前に **`winget source reset --force` + `winget source update`** を実行（昇格 PS1・管理者インライン両経路）。生成 PS1 の構文も検証済み。

### v1.13.6 start.bat を「必須ツールのゲート → アプリ起動」の2段構えに

- **背景**: 標準ユーザーで `start.bat` を実行すると、winget の Python 自動インストールが **1625（組織のポリシー／＝標準ユーザーはマシン全体への MSI 導入不可）** で失敗し、しかも `--silent` 経路のため UAC 昇格も出ずに落ちていた。ツールが揃っていないのに後続（venv 等）へ進む構造も分かりにくかった。
- **修正**: `start.bat` を2フェーズ構成に再設計。
  - **フェーズ1（必須ツールのゲート）**: Python / Git（必須）・Node.js（任意）を検出。不足があれば winget で導入し、**標準ユーザーの場合はインストール部分だけ昇格**（一時 `.ps1` を `Start-Process -Verb RunAs -Wait` で1回だけ管理者起動。UAC は1回）。導入後 PATH をレジストリから再取得して再判定。**Python / Git が揃わなければフェーズ2へ進まず中断**（手動導入の案内＋1625時の注意を表示）。Node.js は未導入でも WARN のみで続行（MCP/Playwright だけ無効）。
  - **フェーズ2**: ゲート通過後にのみ venv 作成・pip・`.env`（ポート対話入力）・Playwright chromium・トレイ起動。
  - 管理者で起動済みなら昇格せずその場で winget 実行（`net session` で判定）。winget 自体が無い環境は手動導入案内で停止。
- **検証**: 生成する昇格用 PS1 の構文（PSParser）OK・ゲート分岐（全部入り→進行 / Git欠落&winget無→中断 / Node欠落→任意扱い）を縮小版で確認。



### v1.13.5 新規インストールの /setup でプロバイダーが2枚出る問題を修正

- **症状**: 新規インストール直後の `/setup` 画面で、「プロバイダーを追加」を押していないのに **Azure OpenAI の下に Azure AI Foundry の入力欄（プレースホルダ）が最初から出ていた**。
- **原因**: `/setup/current`（`server.py`）は `.env` に `FOUNDRY_ENDPOINT` があれば Foundry カードを描画する。新規インストールの `.env` は `.env.example` のコピーで、そこに `FOUNDRY_ENDPOINT=https://your-project...` 等の**プレースホルダが実体行として記載**されていたため、未設定扱いにならずカードが1枚増えていた。
- **修正**: `.env.example` の `FOUNDRY_*`（インスタンス1）をコメントアウト（インスタンス2と同じ扱いに統一）。新規インストールは Azure OpenAI 1枚で始まり、Foundry は `/setup` の「プロバイダーを追加」または `.env` のコメント解除で足す。
- **既存環境は無影響**: 既に Foundry を設定済みの `.env` はそのまま2枚目が出る（意図どおり）。



### v1.13.4 start.bat の初回 .env 作成でポートを選べるように

- **症状**: 新規PCに Windows版をインストールすると、ポートが **8001 ではなく 8000** になっていた。WSL版と同一PCで併用するため Windows版は 8001 が既定の想定だった。
- **原因**: `start.bat` が初回に `.env.example`（中身は `APP_PORT=8000`）をそのままコピーしていたため、`config.py`／`tray.py` の Windows既定 8001 に到達せず、明示された 8000 が使われていた。
- **修正**: `start.bat` の初回 `.env` 作成時に**ポート番号を対話入力**（既定 8001・Enterでそのまま・数値以外は8001にフォールバック）。入力値で `.env` の `APP_PORT=` 行を **UTF-8（BOMなし）** で書き換え（日本語コメントの文字化け防止）。
- **既存インストールの復旧**: `.env` の `APP_PORT=8000` を `APP_PORT=8001` に手書き、または `.env` を削除して `start.bat` 再実行。
...
- **症状**: チャット欄に文字を打ってエンター後、反応までが体感で数秒遅い。
- **原因**: `sendMessage()`（`index.html`）が、本題の `/chat` を呼ぶ前に `/classify-bg` を **await** していた。これは「この依頼は長時間タスク（BG実行）か？」を判定する **Azure OpenAI へのLLM呼び出し**で、入力が15文字超だとほぼ毎回・直列で挟まる（最大5秒）。しかも通常ケース（bg=false）では入力欄クリアもユーザーメッセージ表示も分類完了まで行われず、無反応の沈黙になっていた。
- **修正（案1: UIを先に出す）**: 分類ブロックを **UI描画の後ろ**に移動。エンター直後に入力クリア＋ユーザーメッセージ＋スピナーを表示し、分類は裏で実行。`bg=true`（重いタスク）の稀なケースだけ、出したメッセージを `turnEl.remove()` で撤去して⚡BGカードに差し替え（二重表示なし）。**分類ロジック・精度は不変**——LLM分類は全件そのまま実行している。沈黙が消えるのは「待ち方」を変えただけ。
- タイムアウトを **5秒 → 3秒**に短縮（社内プロキシ環境での最悪値短縮。回線遅延時にBG判定を取りこぼす確率の微増とのトレードオフだが3秒で安全側）。
- **リグレッション対策**: 分類条件に `filesToSend.length === 0`（ファイル添付なし）を追加。これがないと、ファイル添付＋長文で `bg=true` 撤去時に添付ファイルが失われるため。添付ありは安全側で通常実行に倒す。
- **検証**: 通常の長文送信（ユーザー確認）＋ ファイル添付＋長文（Chrome実機で自動テスト）の両経路を確認。沈黙なし・BGカード化けなし・添付消失なし・添付内容も正しく読込、を実DOM＋スクリーンショットで確認済み。

### （参考）setup.sh 実行権限の自動付与フック
...
画像生成と Mermaid 清書（gpt-image-2 img2img）まわりで、運用で詰まっていた点をまとめて解消したセッション。

### v1.10.2 自動ウォーターマークの「2枚保存」問題を修正

- 自動ウォーターマーク適用時、元画像（`generated_*.png`）と透かし入り（`watermarked_*.png`）の **2 枚**が保存されていた。原因は単体ツール `watermark_image` 用の「別名保存」を自動適用パスがそのまま流用していたため。
- `apply_auto_watermark()` に `original_rel_path` を追加し、**元ファイルに上書き**して 1 枚に統合（`server.py` から生成済み `saved_path` を渡す）。`saved_path` は不変で UI/履歴と整合。`edit_image` 経由も同様に 1 枚化。

### v1.11.0 清書のラベル維持機能 ＋ 画像 API 版数の既定値更新

- **清書のラベル維持**（`index.html`）: img2img が元画像の文字を誤読・複製する弱点に対し、Mermaid コードからノード/エッジ/サブグラフのラベルを抽出（`_extractMermaidLabels()`）し、「この文字を一字一句維持・変更/複製/追加/翻訳禁止・要素数と接続関係も維持」という指示を清書プロンプトに自動添付。**「図中の文字を維持」トグル（既定ON）**付き。ラベルを言い換えたい時は OFF。
- **画像 API 版数の既定値更新**: 別 PC（Azure AI Foundry・gpt-image-2）で清書が **HTTP 500**。原因は API バージョンが古い（`2024-02-01`）と img2img(`images.edit`)エンドポイントが存在しないため（生成は動くが編集だけ失敗）。`setup.html` のプレースホルダー・`image_tools.py` のフォールバック既定を **`2025-04-01-preview`** に更新。`.env` に値があるユーザーは不変（既定はあくまで空欄時の保険）。

### v1.11.1 Mermaid 描画の余白改善

- 生 Mermaid のレイアウトが密で重なる問題（清書も悪い元配置を引き継ぐ）への正攻法。`mermaid.initialize` に `flowchart: { nodeSpacing:60, rankSpacing:80, padding:16, curve:'basis' }` を追加。mermaid v10（CDN）のまま確実に効く範囲。**根本改善は ELK レイアウト（v11 化）だが今回は見送り**。

### v1.11.2 AIチェックの無限ループ修正

- AIチェック → 修正適用 → 再チェックで延々と修正案が出続けるループを解消。3 点セット:
  1. **指示文を保守的化**（`server.py`）: 「読みにくいレイアウト」等の主観基準を削除。判読できれば必ず「問題なし」。修正手段も「座標は動かせないので、ラベル短縮・`<br>`改行・方向変更・subgraph化・クロスリンク削減などコードで実際に効くもの」に限定。
  2. **無変更の検出**（`server.py`）: 返ってきた修正コードが元と空白無視で一致なら修正案にしない。
  3. **ループガード**（`index.html`）: 過去の修正案を記憶し、同じ案が再出したら堂々巡りとして打ち切り「これ以上の自動改善はできません（清書での仕上げを推奨）」。ユーザーが明示指摘した再チェック時はガードしない。
- 設計判断: Mermaid は座標制御不可・清書(img2img)は元配置を引き継ぐため、**密で複雑な図の重なりは原理的に直しきれない**。AIチェックは「直せる種類だけ直す」係、清書は「見た目を仕上げる」係、と役割分担。

### v1.12.0 清書ウォーターマークを清書UIのチェックで切替

- 清書画像（資料用途）に無断流用対策の透かしを入れたい要望。当初は設定画面のグローバルトグル（`WATERMARK_REFINE_ENABLED`）で実装したが、**「清書ボタンの入力欄にチェックを置く方が自然・再起動不要」**との指摘で方式変更。
- 清書入力欄に **「ウォーターマークを入れる」チェック（既定ON）** を「図中の文字を維持」と並べて追加。`MermaidRefineRequest.watermark`（既定True）でリクエスト毎に指定 → 再起動不要。
- `apply_refine_watermark(b64, rel_path, enabled)`（`image_tools.py`）で適用。スタイル(文字/色/位置/不透明度/サイズ)は既存 `WATERMARK_*` 設定を共用。透かし入りでも**保存済みファイルに上書きで 1 枚維持**。「もう一度清書」でも有効。
- 撤回したグローバル設定方式（config.py / setup.html / server.py の load/save）は元に戻し済み。

### 両ブランチ同期

- main（`40599e6`）・for_windows（`31938f0`）push 済み。for_windows は main をマージ（コンフリクトなし）。Windows 固有ファイル（start.bat / mcp_servers.json の Playwright 版数固定）は不変。
...
### 追加改善（同セッション・実機フィードバック反映）

初回実装後、実機で触ってもらいながら以下を追加・修正（すべて実機 e2e／構文チェック済み）。

1. **タスク編集 UI** — 一覧に「編集」ボタン。登録フォームを編集モードで再利用（`PUT /schedule/tasks/{id}`）。種別を毎時/N時間ごとに変更した際は anchor_at を現在時刻で自動補完し発火不能を防止。
2. **即時実行ボタン** — 一覧に「▶ 即時実行」。`POST /schedule/tasks/{id}/run-now` でスケジュール無視で即発火。once タスクは即時実行では無効化しない（`_create_job_from_task` を副作用なしに分離）。
3. **作業スコープをプルダウン化** — 自由入力 →「workspace 全体」＋ `/workspace/subdirs` のフォルダ選択。
4. **結果カードの二重表示バグ修正** — スケジューラージョブは既存 BG キューに入るため BG 復元と ⏰ 通知が二重描画していた。`_schedJobIds` で識別し、メインチャットへの保存は ⏰ 側に一本化（`_bgSaveToChat` で抑制）、BG ペインにはリッチ表示を残す。⏰カード出力はテキストチャンクのみに絞り生 JSON 混入を解消。
5. **BG バッジが残るバグ修正** — 上記の BG 除外が原因で 5s ポーリングが状態更新できず badge が stuck。除外を撤回し解消。
6. **結果カードの履歴保存** — スケジューラー結果も `bg_user`/`bg_result` で history に保存しリロード後も残るように。
7. **BG 最大ターン数 30→60** — `config.ASYNC_MAX_TURNS`（`.env` で変更可）に一元化。手動⚡・inbox・定時タスク共通。`agent_core.py`/`async_worker.py`/`server.py` のハードコードを置換。

### 実行記録（フラグ）の保持仕様（確認事項）

- フラグは「task_id × scheduled_at（予定時刻そのもの）」単位。翌日は別 occurrence なので昨日のフラグは今日に影響しない。
- 自動削除は `purge_old_runs(keep_per_task=50)` を毎 tick 実行＝**1タスク50件保持・超過分を古い順に削除**（時間失効ではない）。pending は自動削除しない。
- UI 一覧は最新5件表示。チップの ✕ は flag 解除（記録削除＝未実行扱い）。12h 以内の回を解除すると次 tick で再び取りこぼし確認が出る。
