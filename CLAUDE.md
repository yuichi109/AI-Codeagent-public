...
- **バージョン管理**: `config.py` の `APP_VERSION` を更新する。メジャー=Stage移行、マイナー=機能まとまり完成、パッチ=バグ修正・微調整。コミット時に必ず確認。

---

## セッション開始時に必ず読むメモリファイル

`C:\Users\yuichi.matsuo\.claude\projects\--wsl-localhost-Ubuntu-home-user-AI-Codeagent\memory\` にセッション間の記憶が保存されている。
**セッション開始時に必ず以下を読むこと**（WSL経由: `/mnt/c/Users/yuichi.matsuo/.claude/projects/--wsl-localhost-Ubuntu-home-user-AI-Codeagent/memory/`）：

1. `MEMORY.md` — インデックス（何がどこに書いてあるか）
2. `user_profile.md` — ユーザーの役割・技術背景・作業スタイル
3. `project_status.md` — 現在の実装状況・バージョン・直前の作業
4. `feedback_style.md` — 作業上の指示・好み

その他のファイルは作業内容に応じて適宜参照する。
