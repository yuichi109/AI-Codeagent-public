...
```powershell
git clone -b for_windows https://github.com/yuichi109/AI-Codeagent-public.git AI-Codeagent
cd AI-Codeagent
```
...
5. Playwright chromium をセットアップ（MCP / ブラウザ操作用）
6. **タスクトレイアイコンを起動**（その中でサーバーが常駐。2 回目以降の起動はトレイ起動のみで即座に立ち上がる）
...
| systemd 自動起動 | 非対応 → start.bat を手動起動 |

### サーバーの停止・再起動

- 停止: start.bat のウィンドウで `Ctrl+C`
- 再起動: start.bat を再度ダブルクリック（venv 再作成はスキップ）
...
```powershell
git init
git remote add origin https://github.com/yuichi109/AI-Codeagent-public.git
git fetch origin
...
```bash
git clone git@github.com:yuichi109/AI-Codeagent-public.git AI-Codeagent
cd AI-Codeagent
chmod +x setup.sh   # Windows側エディタで編集すると実行権限が落ちるため、念のため毎回付与
./setup.sh install
```
...
```env
# プロキシバイパス対象ドメイン
no_proxy=*.azure.com,*.openai.azure.com,github.com,gitlab.com,api.duckduckgo.com,ja.wikipedia.org,en.wikipedia.org,localhost,127.0.0.1
NO_PROXY=*.azure.com,*.openai.azure.com,github.com,gitlab.com,api.duckduckgo.com,ja.wikipedia.org,en.wikipedia.org,localhost,127.0.0.1
```
