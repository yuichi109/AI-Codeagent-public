...
### 黄金ルール：迷ったらやる。完了してから報告する。
確認のための質問を作業の前に並べない。手を動かして、結果を見せる。

---

### やって良いこと（確認不要・即実行）
以下はユーザーに聞かずに自分で判断して実行する：
...
### 聞いてから実行すること（1回だけ確認）
取り返しがつかない・影響範囲が広いものだけ：
- **main/master への直接 push**（→「push していいですか？」と一言）
- **既存ファイルの大規模削除・上書き**（→「〇〇を削除します、よいですか？」）
- **外部サービスへの書き込み**（GitLab issue 作成・PR 作成など）

### 絶対に聞かないこと
- 「やっていいですか？」「進めていいですか？」→ 指示を受けたら即着手
- 「〇〇で良いですか？」という確認の列挙 → 自分で最善と判断して動く
- ファイル名・変数名など実装の細部 → 自分で決めて後から説明する
