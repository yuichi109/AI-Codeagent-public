...
- **統計ダッシュボード**（★★）: 利用統計/実行統計を記録・表示する機能。**第1弾＝モデル別利用割合を v1.30.0 で実装済み**（セッション75）。
  - 見たい指標（全部対象）: トークン消費（モデル別・日別）→推定コスト／ジョブ成否率・平均所要時間・ターン数／モデル別利用量・失敗率／スケジューラー成功率・代替フォールバック発生率／（任意）マルチエージェント役割別パフォーマンス。
  - **✅ 実装済み（v1.30.0）**: 別DB `tools/stats_db.py`（`data/stats.db`・`usage_daily` ロールアップ・UPSERT増分・遅延初期化・MAX_USAGE_ROWS=50000トリム）／**チャット＋BG/定時の両経路**で `record_usage()`（agent_core にも stream_options 追加）／`GET /stats/usage?days=`／📊ヘッダーボタン→モーダル（サマリー4枚＋モデル別利用割合の純CSS横棒＋**日別推移グラフ**・回数/トークンの基準切替）。
  - **残（次段）**: ①推定コスト（単価表を config/.env に） ②ジョブ成否率・平均所要時間・ターン数（jobs.db） ③スケジューラー代替フォールバック率（task_runs.actual_model から） ④マルチエージェント役割別パフォーマンス ⑤Power BI 連携用の CSV/Excel エクスポート。
  - **最重要要件＝肥大化させない**（他ユーザーが気にせず使えること）: 生ログを永久保存せず**日次ロールアップ方式**（日×モデル×スコープの集計1行）で行数を使用量から切り離す。別DB `stats.db` に隔離（既存DBに触らない）。← v1.30.0 で踏襲済み。
  - **路線**: A=アプリ内ダッシュボード（純CSS/visualize・無料・一体型／推し・採用）。B=Power BI Desktop（無料・Proは有料）連携用に統計CSV/Excelエクスポート or 集計API を1本用意。**Aを土台にBへ繋げる**。
